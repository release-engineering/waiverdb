...
from sqlalchemy.sql.expression import column, select, table, update
...
def upgrade() -> None:
    ...
    for waiver_id, subject in rows:
        ...
        connection.execute(
            ...
            .values(subject_type=subject_type, subject_identifier=subject_identifier)
        )
    ...
    op.create_index(
        'ix_waiver_subject_type_identifier',
        'waiver',
...
def downgrade() -> None:
    ...
    rows = connection.execute(
        select(
            waiver_table.c.id,
            waiver_table.c.subject_type,
            waiver_table.c.subject_identifier,
